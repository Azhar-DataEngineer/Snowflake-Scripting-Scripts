...
CREATE OR REPLACE PROCEDURE EMP.PROCS.SP_EXCEPTION_HANDLING_DEMO("COUNTRY" VARCHAR)
    ...
    AS CALLER
    AS DECLARE no_data_found EXCEPTION (-20101, 'No Data Found in Employee Table');
cnt INTEGER;
...
-- lec := loc_emp_count;
    ...
    WHEN expression_error THEN RETURN OBJECT_CONSTRUCT(
        'Error type',
        'EXPRESSION ERROR',
        'SQLCODE',
        sqlcode,
    ...
    WHEN no_data_found THEN RETURN OBJECT_CONSTRUCT(
        'Error type',
...
CALL EMP.PROCS.SP_EXCEPTION_HANDLING_DEMO('IND');
== == == == == == == == == == == == == == == == == == == == == ==
-- Inserting the errors into a log table
CREATE SCHEMA IF NOT EXISTS EMP.WORK;
CREATE
OR REPLACE TABLE EMP.WORK.SP_ERROR_LOGS (
    PROC_NAME VARCHAR,
    ERROR_TYPE VARCHAR,
    ERROR_CODE VARCHAR,
